<template>
  ...
  <PInput v-model="item.result" margin="0" label="Результат анализа" placeholder="Введите результат анализа" />
...
const item = ref(AnalyzeAcids.Create(props.patient.id));
const emit = defineEmits(['create']);

const create = async () => {
  await AnalyzesAcidsStore.Create(item.value);
  props.patient.addAnalyzeAcids(item.value);
  emit('create');
};
</script>
<style scoped>
